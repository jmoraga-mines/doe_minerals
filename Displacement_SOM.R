...
pacman::p_load(raster, sf, xlsx, readxl)
library(sf)
library(raster)
library(xlsx)
library(readxl)
...
brady_csv <- ("D:/DOE/Papers/Data/Brady_72img_TS_7.csv")
brady_csv_data <- read.csv("D:/DOE/Papers/Data/Brady_72img_TS_7.csv")
df_csv <- as.data.frame(brady_csv_data)

# Basemap (r)
bra_bor <- st_read("D:/DOE/Data/VectorData/BradysVectorData/BradyNewFrameElena.shp")
bra_extent <- extent(bra_bor)
bra_extent_ras <- raster(bra_extent, 
...
#Run file_csv function here  / Inclunce area and ther packages shoudl be downpladed
